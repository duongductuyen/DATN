...
class OrderController extends Controller
{
	...
	public function print_order_convert($checkout_code){
		...
		<p style="font-style: italic; text-align: center; margin: -20px 0 20px 0; font-size: 14px;">(Bản thể hiện của hóa đơn điện tử)</p>
		<hr>
		<b>CÔNG TY TNHH MỘT THÀNH VIÊN CYBER-T</b>
		<p>Mã số thuế: 0102030405</p>
		<p>Địa chỉ: Số 298 Đ. Cầu Diễn, Minh Khai, Bắc Từ Liêm, Hà Nội</p>
		<p>Điện thoại: 0240123456</p>
		<hr>
		';		
		$output.='		
		<p>Họ tên người mua hàng: '.$customer->customer_name.'</p>
		<p>Số điện thoại: 0'.$customer->customer_phone.'</p>
		...
		';	
		$output.='
		<p>Họ tên người nhận hàng: '.$shipping->shipping_name.'</p>
		...
		$output.='		
					<tr>
						<td style="text-align: center; ">'.$keyy.'</td>
				...
				if(strpos($coupon_echo, '%') == true){
					$coupon_echo = $total_after_coupon;
				}
		...
		$output.= '
				<tr>
					<td colspan="6">
						...
						<p>Giảm giá: '.number_format($coupon_echo,0,',','.').'đ'.'</p>
						<p>Tổng tiền thanh toán: '.number_format($total_coupon + $product->product_feeship,0,',','.').'đ'.'</p>
					</td>
				</tr>
				<tr>
		...
		$output.='
			...
			<table class="table-stylingg">
				<thead>
					<tr>
						<th width="200px"><p>Người bán hàng</p></th>
						<th width="800px"><p>Người nhận hàng</p></th>
					</tr>
					<tr>
						<td width="200px"><p style="font-style: italic; text-align: center; margin-top: -5px;">(Ký, ghi rõ họ, tên)</p></td>
						<td width="800px"><p style="font-style: italic; text-align: center; margin-top: -5px;">(Ký, ghi rõ họ, tên)</p></td>
					</tr>
				</thead>
				<tbody>';
    ...
    public function manage_order(){
        ...
        $contact = Contact::where('info_id',1)->get();
			...
			->with('contact',$contact)->with('getorder',$getorder)->with('category_post',$category_post); //1
			}
		...
		->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)->with('slider',$slider)
		->with('contact',$contact)->with('getorder',$getorder)->with('order_details',$order_details)->with('getcustomer',$getcustomer)
		->with('shipping',$shipping)->with('coupon_condition',$coupon_condition)->with('getorder',$getorder)->with('order_status',$order_status)
		->with('coupon_number',$coupon_number)->with('order_code',$order_code)->with('category_post',$category_post); //1
			}
		}	
